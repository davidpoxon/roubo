...
// file reached main unguarded. This test closes that gap by validating every real
// envelope-format spec file, guarding all current and future ones against the same
// defect class. The strict contract types `tracker.ref` / `tracker.blocked_by_refs[]` /
...
const specsRoot = resolve(repoRoot, "docs", "specifications");
...
// Discover every committed envelope-format `docs/specifications/<slug>/work-units.json`.
...
describe("committed work-units.json files satisfy the strict contract", () => {
  it("discovers at least one committed envelope-format work-units.json", () => {
    expect(
      specFiles.length,
      `expected to find at least one envelope-format docs/specifications/<slug>/work-units.json under ${specsRoot}, found none`,
    ).toBeGreaterThan(0);
  });

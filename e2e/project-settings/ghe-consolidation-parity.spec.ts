...
// IP-TC-179 (IP-US-023, IP-FR-073): GHE follows the same Source-tile consolidation as
...
// Submodules editors are gated on plugin.id === "github-com" today (IP-FR-073
// follow-up), so this spec deliberately does not assert their presence on
// the GHE modal; see the IP-WU-068 plan for the carve-out.
...
test("section title and instance URL reflect the GHE plugin manifest and effective config", async ({
  ...
  // IP-FR-069: GHE's manifest.name surfaces in the Source section title.
  await expect(page.getByTestId("project-settings-source-section-title")).toHaveText(
    "GitHub Enterprise",
  );

  // IP-FR-073: the configured instance URL is rendered as a read-only line on
  ...
  // identical to the github-com case (IP-FR-072 parity).

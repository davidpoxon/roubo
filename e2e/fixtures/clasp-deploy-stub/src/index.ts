...
// Imperative (escape-hatch) component plugin used by the CP-TC-028 e2e drift
// guard. It models a one-shot "deploy" lifecycle a declarative
// ProvisionDescriptor cannot express: the start hook queries the host for a
// capability, runs a single command to completion through the host broker, and
// reports `completed`. Every process and container handle is owned by the host;
// this plugin only drives the broker over JSON-RPC.
//

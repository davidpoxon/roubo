...
- **`lint:design-tokens`** fails if `client/src` paints with a raw palette colour utility (`text-stone-500`, any family, any colour prefix, any variant) or any `dark:` colour variant, instead of a [DESIGN.md](DESIGN.md) semantic role utility. Unmigrated paths are skipped via `scripts/design-token-allowlist.json`, which only shrinks: an entry that covers no violation fails as stale, so migrating a path means deleting its entry in the same change. Guard: `scripts/design-token-guard.mjs`. The "which token when" rule is in [docs/development.md](docs/development.md#design-token-gates).
...
- Delegate narrowly. Subagents are for read-only exploration you would otherwise do serially across unrelated areas; cap at three concurrent for one task. Implementation, refactors, and anything that writes to the tree stay in this thread. The pre-push review below is the one other use: a session cannot review code it just wrote.

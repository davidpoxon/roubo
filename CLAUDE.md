...
- **`lint:dep-pins`** fails if any dependency spec is not an exact version, or if `package-lock.json` records a spec that differs from the `package.json` it mirrors. The second rule is the one no other gate can see: Dependabot writes an exact pin into the manifest but a caret into the lock, and `npm ci` accepts that because the recorded range still satisfies the manifest. Four such drifts merged green before this gate existed. The repo's own workspace packages are exempt, since they are linked rather than fetched. Guard: `scripts/dependency-pin-guard.mjs`.
- **`lint:design-tokens`** fails if `client/src` or `client/index.html` paints with a raw palette colour utility (`text-stone-500`, any family, any colour prefix, any variant) or any `dark:` colour variant, instead of a [DESIGN.md](DESIGN.md) semantic role utility. The colour migration (#1293) is complete, so `scripts/design-token-allowlist.json` is empty; keep it that way, since an entry that covers no violation fails as stale and a new one must never be used to get a violation past the gate. Guard: `scripts/design-token-guard.mjs`. The "which token when" rule is in [docs/development.md](docs/development.md#design-token-gates).
- **`lint:semantic-dark`** fails if DESIGN.md's token block records a `-dark` colour that `design-tokens/semantic-dark.css` does not switch to under `.dark`. Nothing else catches it: the build compiles and the light value renders in dark mode. Guard: `scripts/semantic-dark-guard.mjs`.
- **`lint:emitted-tokens`** fails if DESIGN.md's token block and the generated `design-tokens/tokens.tailwind.css` disagree, in either direction or on a value. That file is written by the external ui-design `emit_tokens.py` and committed by hand, and nothing else looks at both: `lint:design-tokens` reads the legal token names out of it, so a name that never arrived is simply not a token and its absence is invisible. The build compiles and the utility paints nothing. Regenerate with the emitter rather than editing the file. Guard: `scripts/emitted-tokens-guard.mjs`.
...
- **`lint:repo-refs`** fails if any tracked file, of any type, names a repository this public repository must not reference. The disallowed names are stored only as SHA-256 digests. Cite spec case and requirement ids or this repository's own issue and PR numbers, and use a neutral fixture project id (`demo`). FR-020 journey guards name their owning slices by title for the same reason. Guard: `scripts/repo-reference-guard.mjs`.

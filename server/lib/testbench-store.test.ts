...
describe("readPlanAndResults", () => {
  // TC-038/039: results key per project+spec+bench under .specifications/<slug>.
  it("reads the plan and resolves results from the per-bench sidecar key", async () => {
    writePlan(planFor());
    await markObservation(repo, SLUG, BENCH, "TC-001", "O1", "pass");
    await markObservation(repo, SLUG, "bench-2", "TC-001", "O1", "fail");
    ...
    // The OTHER bench's mark is keyed separately and not leaked into this view.
    const other = readPlanAndResults(repo, SLUG, "bench-2");
    expect(other.results?.caseResults["TC-001"].observationMarks.O1.result).toBe("fail");

    // One sidecar file holds both benches.
  ...
  it("throws MissingPlanError when the plan is absent", () => {
    expect(() => readPlanAndResults(repo, SLUG, BENCH)).toThrow(MissingPlanError);
  });
  ...
  it("fails open on a schema-invalid sidecar", () => {
    writePlan(planFor());
    writeRawResults(JSON.stringify({ benches: "not-a-record" }));
    const view = readPlanAndResults(repo, SLUG, BENCH);
    expect(view.results).toBeNull();
    expect(view.recovered).toBe(true);
  });
  ...
  it("flags stale when the plan changes after results were written", async () => {
    ...
    const fresh = readPlanAndResults(repo, SLUG, BENCH);
    ...
    const after = readPlanAndResults(repo, SLUG, BENCH);
...
describe("path safety (NFR-001)", () => {
  ...
  it("rejects traversal and dot slugs", () => {
    for (const bad of ["../../etc/evil", "..", ".", "../outside"]) {
      expect(() => readPlanAndResults(repo, bad, BENCH)).toThrow(UnsafePathError);
    }
  });

  // TC-052: a slug carrying a separator that escapes the repo is rejected.
  it("rejects a slug with a path separator", () => {
    expect(() => readPlanAndResults(repo, "a/b", BENCH)).toThrow(UnsafePathError);
  });

  // Writers reject too, before any fs mutation.
  it("rejects an unsafe slug on write paths", async () => {
    await expect(markObservation(repo, "../evil", BENCH, "TC-001", "O1", "pass")).rejects.toThrow(
      UnsafePathError,
    );
    await expect(appendNote(repo, "../evil", BENCH, "TC-001", "hi")).rejects.toThrow(
      UnsafePathError,
    );
  });
});
...
describe("markObservation", () => {
  ...
  it("upserts a mark and recomputes derivedStatus", async () => {
    let result = await markObservation(repo, SLUG, BENCH, "TC-001", "O1", "pass");
    expect(result.derivedStatus).toBe("in_progress");

    result = await markObservation(repo, SLUG, BENCH, "TC-001", "O2", "pass");
    expect(result.derivedStatus).toBe("passed");

    // A fail flips it.
    result = await markObservation(repo, SLUG, BENCH, "TC-001", "O2", "fail");
  ...
  it("stamps the resolved git identity on the mark", async () => {
    const result = await markObservation(repo, SLUG, BENCH, "TC-001", "O1", "pass");
  ...
  it("never mutates the source plan (byte-identical after write)", async () => {
    ...
    await markObservation(repo, SLUG, BENCH, "TC-001", "O1", "pass");
    await markObservation(repo, SLUG, BENCH, "TC-001", "O2", "fail");
...
describe("appendNote", () => {
  ...
  it("appends an immutable note stamped with author, timestamp, and statusAtWrite", async () => {
    await markObservation(repo, SLUG, BENCH, "TC-001", "O1", "pass");
    const note = await appendNote(repo, SLUG, BENCH, "TC-001", "  Looks good  ");
  ...
  it("captures an override status in statusAtWrite", async () => {
    await setStatusOverride(repo, SLUG, BENCH, "TC-001", "blocked");
    const note = await appendNote(repo, SLUG, BENCH, "TC-001", "Blocked by infra");
    expect(note.statusAtWrite).toBe("blocked");
  });

  it("rejects empty or whitespace-only note text", async () => {
    await expect(appendNote(repo, SLUG, BENCH, "TC-001", "")).rejects.toThrow(/empty/);
    await expect(appendNote(repo, SLUG, BENCH, "TC-001", "   ")).rejects.toThrow(/empty/);
  });

  it("never mutates the source plan (byte-identical after note)", async () => {
    const before = planBytes();
    await appendNote(repo, SLUG, BENCH, "TC-001", "a note");
    expect(planBytes().equals(before)).toBe(true);
  });
});
...
describe("setStatusOverride", () => {
  ...
  it("sets and clears an override", async () => {
    let result = await setStatusOverride(repo, SLUG, BENCH, "TC-001", "blocked");
    expect(result.statusOverride?.status).toBe("blocked");
    expect(result.statusOverride?.author).toEqual(REAL_IDENTITY);

    result = await setStatusOverride(repo, SLUG, BENCH, "TC-001", null);
...
describe("reconcile (NFR-003 orphan-not-delete)", () => {
  ...
  it("previews classification without writing when confirm is not set", async () => {
    ...
    const outcome = await reconcile(repo, SLUG, BENCH);
  ...
  it("flags orphans without deleting them when confirmed (orphan-not-delete)", async () => {
    ...
    const outcome = await reconcile(repo, SLUG, BENCH, { confirm: true });
    ...
    expect(file.benches[BENCH].caseResults["TC-001"].orphaned).toBe(true);
    expect(file.benches[BENCH].caseResults["TC-001"].observationMarks.O1.result).toBe("pass");
  });
  ...
  it("physically deletes orphans only when purgeOrphans is explicitly true", async () => {
    ...
    await reconcile(repo, SLUG, BENCH, { confirm: true, purgeOrphans: true });
    const file: TestResultsFile = JSON.parse(fs.readFileSync(resultsFilePath(), "utf8"));
    expect(file.benches[BENCH].caseResults["TC-001"]).toBeUndefined();
  });

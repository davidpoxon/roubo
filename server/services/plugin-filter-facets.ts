 ...
 * drops the ones that fail (IP-FR-065, IP-TC-190) so malformed descriptors never
 ...
 * `MethodNotFound`. Per IP-FR-065 / IP-TC-126, core never surfaces
 ...
 * IP-FR-065). Returns the plugin's facet descriptors after dropping any that fail
...
export async function getPluginFilterFacets(pluginId: string): Promise<FilterFacet[]> {
  ...
  // on `.filter`, so malformed output never reaches the client (IP-FR-065).
 ...
 * 1.1.0+, IP-FR-065). Plugins built against 1.0.0, or plugins that omit the

...
// plugin-host-api.ts so every host surface speaks one denial code (F2.1, #673).
...
// names the method and, when known, its minimum host version (#893, FR-017).
 ...
 * This is the forward-compat extension point (#893): when a later additive
 ...
 * crash cleanup and the startup orphan sweep can reap it (#887, AC4).
 */
 ...
 * call, #687), so the resolver routes the call to the exact bench by that key
...
/**
 * Build the broker's descriptive method-not-found error (#893, FR-017).
...
/**
 * Permission gate (F2.1, #673). Throws a permission-denied ResponseError when
 ...
 * with (#894). `code: "process-timeout"` lets a caller (or the SDK) distinguish a
 ...
 *
 * Permission is enforced (F2.1, #673): each handler calls enforcePermission
...
export function registerBrokerHandlers(
  ...
  // rejection on the connection (#887).
  ...
  connection.onRequest<ProcessStartParams, { pid: number }>(
    ...
    async (params) => {
      ...
      try {
        ...
        // reap it (#887, AC4). Recorded after a successful spawn: the process is
  ...
  connection.onRequest<ProcessRunParams, { exitCode: number }>(
    ...
    async (params) => {
      ...
      // ledger entry the cleanup sweep can reap (#887, AC4), mirroring the
      ...
      // cannot distinguish from a genuine exit(124) (#894). Reject with a typed,
  ...
  // notification form too (#887, AC2). The notification carries no `name` (the
  ...
  // minimum host version that provides it (#893, FR-017).

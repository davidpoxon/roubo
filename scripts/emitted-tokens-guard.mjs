...
// The generated file is a single `@theme { ... }` block between the managed
// region sentinels. No declaration in it nests a brace (`rgb(...)` and
// `cubic-bezier(...)` are parenthesised), so a lazy match to the first
// column-zero `}` takes the whole block and nothing more.
 ...
 * by name. A declaration outside that block does not count.
 *
 * @param {string} css - tokens.tailwind.css contents.
 * @returns {Map<string, string>}
 */
export function themeDeclarations(css) {
  const block = THEME_BLOCK.exec(css);
  if (!block) {
    throw new Error(
      `${TAILWIND_PATH} has no \`@theme { ... }\` block. It is generated by the ` +
        "ui-design emit_tokens.py; regenerate it rather than editing it by hand.",
    );
  }

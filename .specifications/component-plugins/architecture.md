...
| Marketplace                 | service    | new (v3)                  | Registry-backed browse/search/install/update of first-party-curated plugins, integrity-verified, with revocation. The marketplace UI meets WCAG 2.1 AA and is built with React Aria Components (NFR-007).                                                         |

## Data model

| Entity                     | Owner                        | Shape                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| -------------------------- | ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| ProvisionDescriptor        | ProvisionDescriptor (shared) | `{ schemaVersion: number } & ( { kind: "docker", composeFile: string, service: string, initService?: string, portEnvVar?: string, migration?: { command: string, args?: string[] }, connection?: { template: string }, assignedContainerId?: string, healthcheck?: boolean } \| { kind: "process", command: string, env?: Record<string,string>, envFile?: string, cwd?: string, setup?: string, dependsOn?: string[] } \| { kind: "oneshot", command: string, env?: Record<string,string>, envFile?: string, cwd?: string, dependsOn?: string[], timeoutMs?: number } )` |
...
PRD-supplied invariants: zero orphaned resources (NFR-003), held by the ledger plus a startup sweep; zero component-type literals / core docker-field branches (NFR-006), held by the CI guard; component plugins are spawned **once per plugin** (not per bench), so a single process multiplexes benches via `BenchContext.benchId`. The `envFile?` field on the `process` / `oneshot` descriptor is read and merged into `env` by the `LifecycleEngine` (not by the plugin's pure `translate`), preserving the FR-005 / FR-007 env/envFile injection parity.
...
- (v2) `GET /api/plugins/:pluginId/audit -> AuditEntry[]` and `GET /api/projects/:projectId/benches/:id/audit -> AuditEntry[]`: the per-plugin and per-bench privileged-call audit query (FR-019), backed by `AuditLog`.
...
1-2. As above. 3. The plugin implements the imperative hooks; `LifecycleEngine` calls `component.start(BenchContext)`. 4. Inside `start`, the plugin orchestrates via broker calls (for a Clasp deploy: `host.process.run({ command: "clasp push", timeoutMs })`, possibly preceded by an OAuth/project-selection step). The host owns the spawned process/container; the ledger records it. 5. For a one-shot deploy, `host.process.run` resolves with an exit code and the plugin reports `ComponentStatus.status = "completed"`. The blend resolves the god-schema risk: a simple deploy fits a `oneshot` descriptor; a complex one uses the broker escape hatch.
...
- [x] Coarse vs fine broker granularity for docker (a single `host.docker.startService` that runs the whole phase machine vs separate `composeUp` / `waitForHealthy` calls): the engine uses coarse internally for NFR-002; the broker exposes the fine-grained calls for the escape hatch. **Resolved by spike #600 (SPK-3):** the broker exposes the fine-grained calls only; the `LifecycleEngine` sequences them coarsely in-host; there is no `host.docker.startService` RPC.
- [x] External container assignment: a `assignedContainerId` field on `DockerDescriptor` (declarative) and/or a `host.docker.assignContainer` broker method gated on the `docker` permission (imperative). **Resolved by spike #600 (SPK-3):** both, per path (the lean is confirmed); a manifest capability flag is rejected.
- [x] The v2 isolation backend (Apple container framework vs Virtualization.framework vs Docker vs Linux namespaces): resolved by the sandboxing spike; it also answers whether Docker must be present. **Resolved by spike #599 (SPK-2):** capability-broker primary with an opt-in container/VM tier (Virtualization.framework / Apple container / Docker, highest-isolation-first); Docker is not required.
- [ ] One SDK package exposing `defineComponentPlugin()` alongside `definePlugin()`, vs splitting into separate workspaces.
- [x] `host.capability.query` covers a plugin probing the host; the inverse (the host detecting which contract methods a plugin implements before calling them) is handled by `contractVersion` + registered-method introspection: confirm a plugin missing `stop` is rejected at validation, not at stop-time. **Resolved by spike #600 (SPK-3):** confirmed, the host validates the registered-method set against the declared `contractVersion` mode at validation time, so a plugin missing `stop` is rejected then, not at stop-time.

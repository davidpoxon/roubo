...
- [x] Coarse vs fine-grained component lifecycle contract: a single blocking `host.docker.startService` (returns when healthy) versus separate `composeUp` / `waitForHealthy` calls. Coarse is simpler; fine-grained composes better for novel types like a Clasp deploy (FR-022). **Resolved by spike #600 (SPK-3):** the broker exposes the fine-grained `composeUp` / `waitForHealthy` / `composeRunInit` / `composeStop` / `composeDown` calls; the `LifecycleEngine` sequences them coarsely in-host to hold the NFR-002 budget; there is no `host.docker.startService` RPC.
- [x] Does `assignContainer` (today guarded by `type === "database"`) become a manifest capability flag, or is it inferred from a declared `docker` permission category? (Affects FR-006, FR-011.) **Resolved by spike #600 (SPK-3):** both, per path, an `assignedContainerId` field on the `DockerDescriptor` (declarative) and a `host.docker.assignContainer` broker method gated on the `docker` permission (imperative); a manifest capability flag is rejected.
...
- [x] Does the v2 isolation backend need Docker present, or can the Apple container framework / Virtualization.framework satisfy NFR-001/FR-018 on macOS without a Docker dependency? **Resolved by spike #599 (SPK-2):** Docker is not required; the capability-broker is the primary enforcement floor, with an opt-in container/VM tier (Virtualization.framework / Apple container / Docker, highest-isolation-first) where the host supports it.

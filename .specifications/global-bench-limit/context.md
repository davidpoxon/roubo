...
- App-wide Settings UI lives in `client/src/components/ProjectSettings.tsx` with tabs: `benches` (currently labelled "Bench Defaults"), `appearance`, `blueprints`, `integrations`, `plugins`, `claude-code`. Tab labels are in `TAB_LABELS` at line 712 — rename `benches` → `"Benches"`.

...
describe("JigMarkdownEditor", () => {
  it("renders the editor container", () => {
    render(<JigMarkdownEditor value="" onChange={() => {}} />);
    expect(screen.getByTestId("jig-markdown-editor")).toBeInTheDocument();
  });

  it("calls onChange when content changes via insertAtCursor", () => {
    const onChange = vi.fn();
    const ref = createRef<JigMarkdownEditorRef>();
    render(<JigMarkdownEditor ref={ref} value="" onChange={onChange} />);
  ...
  it("replaces selected text when insertAtCursor is called", () => {
    const onChange = vi.fn();
    const ref = createRef<JigMarkdownEditorRef>();
    render(<JigMarkdownEditor ref={ref} value="hello world" onChange={onChange} />);
  ...
  it("exposes a focus method", () => {
    const ref = createRef<JigMarkdownEditorRef>();
    render(<JigMarkdownEditor ref={ref} value="" onChange={() => {}} />);

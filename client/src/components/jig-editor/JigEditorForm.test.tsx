...
vi.mock("../../lib/api", () => ({
  ...
  isJigReferencedError: () => false,
}));

vi.mock("./JigMarkdownEditor", () => ({
...
vi.mock("./JigPreviewPanel", () => ({
  default: ({ content }: { content: string }) => (
    <div data-testid="jig-preview-panel">{content}</div>
  ),
}));

vi.mock("./JigIconPicker", () => ({
...
function renderForm(props: Partial<Parameters<typeof JigEditorForm>[0]> = {}) {
  const defaults: Parameters<typeof JigEditorForm>[0] = {
    mode: "create",
    scope: "global",
  };
  return render(
    <MemoryRouter>
      <JigEditorForm {...defaults} {...props} />
    </MemoryRouter>,
  );
}

describe("JigEditorForm", () => {
  beforeEach(() => {
  ...
  it("renders in create mode with empty fields", () => {
    ...
    expect(screen.getByPlaceholderText("My jig")).toHaveValue("");
    expect(screen.getByPlaceholderText("What this jig does")).toHaveValue("");
    expect(screen.getByRole("button", { name: "Save" })).toBeInTheDocument();
  });

  it("populates fields from initial in edit mode", () => {
    renderForm({ mode: "edit", initial: initialJig });
    expect(screen.getByPlaceholderText("My jig")).toHaveValue("My Jig");
    expect(screen.getByPlaceholderText("What this jig does")).toHaveValue("A test jig");
  });

  it("shows the jig id in edit mode", () => {
    renderForm({ mode: "edit", initial: initialJig });
  ...
  it("shows ID preview in create mode when name is typed", async () => {
    ...
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "Test Jig");
    expect(screen.getByText(/ID will be: test-jig/)).toBeInTheDocument();
  });
  ...
  it("blocks Save when description is empty", async () => {
    const user = userEvent.setup();
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "Some Name");
    await user.click(screen.getByRole("button", { name: "Save" }));
  ...
  it("shows inline error when name exceeds 100 characters", async () => {
    const user = userEvent.setup();
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "a".repeat(101));
    await user.click(screen.getByRole("button", { name: "Save" }));
  ...
  it("shows inline error when description exceeds 300 characters", async () => {
    const user = userEvent.setup();
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "Valid Name");
    fireEvent.change(screen.getByPlaceholderText("What this jig does"), {
  ...
  it("calls mutateAsync and navigates on successful create", async () => {
    ...
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "New Jig");
    await user.type(screen.getByPlaceholderText("What this jig does"), "Does things");
  ...
  it("calls update mutateAsync in edit mode", async () => {
    ...
    await waitFor(() =>
      expect(mockUpdate).toHaveBeenCalledWith({
        id: "my-bp",
        body: expect.objectContaining({ name: "My Jig" }),
      }),
    );
  });
  ...
  it("blocker predicate returns false after a successful save (justSavedRef prevents re-block)", async () => {
    ...
    const loc = { pathname: "/settings/jigs/new" };
  ...
  it("shows soft-warn copy and updates percent when content approaches 50 KB", async () => {
    ...
    expect(screen.getByText(/Large jig/)).toBeInTheDocument();
  ...
  it("shows size-limit error and blocks Save when content exceeds 200 KB", async () => {
    ...
    renderForm({ mode: "create" });
    await user.type(screen.getByPlaceholderText("My jig"), "My Jig");
    await user.type(screen.getByPlaceholderText("What this jig does"), "Does things");
    fireEvent.change(screen.getByTestId("markdown-editor"), {
  ...
  it("breadcrumb shows the current icon for create mode", () => {
    renderForm({ mode: "create" });
    // JigIcon renders a lucide SVG; check it exists in the breadcrumb area
  ...
  it("breadcrumb icon reflects picker selection", async () => {
    ...
    // After picking rocket, the picker current-icon and breadcrumb JigIcon should update
  ...
  it("shows inline name error for DUPLICATE_NAME API response", async () => {
    ...
    await user.click(screen.getByRole("button", { name: "Save" }));
    await waitFor(() =>
      expect(screen.getByText("Another jig already uses this name.")).toBeInTheDocument(),
    );
  });
  ...
  it("shows the markdown editor in the Edit tab by default", () => {
    ...
    expect(screen.queryByTestId("jig-preview-panel")).not.toBeInTheDocument();
  });
  ...
  it("shows the preview panel when Preview tab is clicked", async () => {
    ...
    expect(screen.getByTestId("jig-preview-panel")).toBeInTheDocument();
  ...
  it("passes current content to the preview panel", async () => {
    ...
    renderForm({ mode: "edit", initial: initialJig });
    await user.click(screen.getByRole("tab", { name: "Preview" }));
    expect(screen.getByTestId("jig-preview-panel")).toHaveTextContent("# Hello world");
  });

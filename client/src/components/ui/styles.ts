...
const BUTTON_BASE_CLASSES = `inline-flex items-center justify-center gap-1.5 rounded-control text-13 font-medium transition-colors ${disabledControl} ${focusRingOffset}`;
...
const NAV_BASE_CLASSES = `flex items-center rounded-control px-3 py-1.5 text-13 transition-colors ${disabledControl} ${focusRing}`;
...
export const INPUT_CLASS =
  "w-full rounded-control border border-border-control bg-bg-field px-4 py-2 text-13 text-text-primary placeholder:text-text-secondary transition-colors outline-none focus:border-focus-ring focus:ring-2 focus:ring-focus-ring data-[focused]:border-focus-ring data-[focused]:ring-2 data-[focused]:ring-focus-ring aria-[invalid=true]:border-danger data-[invalid]:border-danger disabled:opacity-40 disabled:cursor-default";

/** Paths and commands typed into a field are set in mono. */
export const INPUT_MONO_CLASS = "font-mono";

export const FIELD_LABEL_CLASS = "block text-11 font-medium text-text-secondary mb-1.5";

export const FIELD_ERROR_CLASS = "mt-1.5 text-12 text-danger-text";

...
export default function GateStatePanel({ gate }: { gate: GateState }) {
  ...
  return (
    <section
      ...
      className="rounded-lg ring-1 ring-inset ring-border bg-bg-base px-4 py-3 flex flex-col gap-3"
    >
      ...
      {isNoGatingCases ? (
        ...
        <p className="text-12 text-text-secondary">All gating cases passed. Nothing outstanding.</p>
      ) : (
        <div className="flex flex-col gap-3">
          <div className="flex flex-col gap-1.5">
            <p className="text-11 uppercase tracking-label text-text-secondary">Unresolved cases</p>
            {unresolved.length > 0 ? (
              <ul className="flex flex-wrap gap-1.5">
                {unresolved.map((caseId) => (
                  <li
                    key={caseId}
                    className="font-mono text-11 text-text-body rounded-md bg-bg-hover px-1.5 py-0.5"
                  >
                    {caseId}
                  </li>
                ))}
              ...
              <p className="text-12 text-text-secondary">None.</p>
            )}
          </div>

          <div className="flex flex-col gap-1.5">
            <p className="text-11 uppercase tracking-label text-text-secondary">Covering units</p>
            {covering.length > 0 ? (
              <ul className="flex flex-wrap gap-1.5">
                {covering.map((unitId) => (
                  <li
      ...
      {lifecycleExcluded.length > 0 && (
        <p data-testid="gate-lifecycle-excluded" className="text-12 text-text-secondary">
          {"Excluded by lifecycle: "}
          <span className="font-mono text-text-body">{lifecycleExcluded.join(", ")}</span>

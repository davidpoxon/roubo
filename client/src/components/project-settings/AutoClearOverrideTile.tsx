...
function toRadioValue(autoClear: boolean | null): AutoClearValue {
  if (autoClear === true) return "on";
  if (autoClear === false) return "off";
  return "inherit";
}

function fromRadioValue(val: AutoClearValue): boolean | null {
  if (val === "on") return true;
  if (val === "off") return false;
  return null;
}

export function AutoClearOverrideTile({
  draft,
  onChange,
}: {
  draft: boolean | null;
  ...
  const appDefault = settings?.benches?.autoClear ?? DEFAULT_BENCH_SETTINGS.autoClear;
  ...
  return (
    <Tile
      icon={<BrushCleaning aria-hidden="true" size={14} />}
      title="Auto-clear"
      ariaLabel="Auto-clear override"
      data-testid="auto-clear-override-tile"
    ...
    >
      ...
      <RadioGroup
        value={selectedValue}
        onChange={(val) => onChange(fromRadioValue(val as AutoClearValue))}

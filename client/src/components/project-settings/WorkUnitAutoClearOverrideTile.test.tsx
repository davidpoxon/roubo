...
import { WorkUnitAutoClearOverrideTile } from "./WorkUnitAutoClearOverrideTile";
...
function renderTile(draft: boolean | null = null, onChange = vi.fn()) {
  return renderWithProviders(
    <MemoryRouter>
      <WorkUnitAutoClearOverrideTile draft={draft} onChange={onChange} />
    </MemoryRouter>,
  );
}
...
beforeEach(() => {
  ...
  mockedUseSettings.mockReturnValue({
    settings: { benches: { workUnitAutoClear: true } },
    isLoading: false,
    updateSettings: vi.fn(),
  } as unknown as ReturnType<typeof useSettings>);
});

describe("WorkUnitAutoClearOverrideTile", () => {
  ...
  it("shows App default: off when app default is false", () => {
    mockedUseSettings.mockReturnValue({
      settings: { benches: { workUnitAutoClear: false } },
      isLoading: false,
      updateSettings: vi.fn(),
    } as unknown as ReturnType<typeof useSettings>);
    renderTile();
  ...
  it("uses DEFAULT_BENCH_SETTINGS.workUnitAutoClear when settings are unavailable", () => {
    mockedUseSettings.mockReturnValue({
      settings: undefined,
      isLoading: true,
      updateSettings: vi.fn(),
    ...
    renderTile();
    const expectedHint = DEFAULT_BENCH_SETTINGS.workUnitAutoClear
      ? "App default: on"
      : "App default: off";

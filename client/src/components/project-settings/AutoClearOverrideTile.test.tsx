...
import { AutoClearOverrideTile } from "./AutoClearOverrideTile";
...
function renderTile(draft: boolean | null = null, onChange = vi.fn()) {
  return renderWithProviders(
    <MemoryRouter>
      <AutoClearOverrideTile draft={draft} onChange={onChange} />
    </MemoryRouter>,
  );
}
...
beforeEach(() => {
  ...
  mockedUseSettings.mockReturnValue({
    settings: { benches: { autoClear: true } },
    isLoading: false,
    updateSettings: vi.fn(),
  } as unknown as ReturnType<typeof useSettings>);
});

describe("AutoClearOverrideTile", () => {
  ...
  it("shows App default: off when app default is false", () => {
    mockedUseSettings.mockReturnValue({
      settings: { benches: { autoClear: false } },
      isLoading: false,
      updateSettings: vi.fn(),
    } as unknown as ReturnType<typeof useSettings>);
    renderTile();
  ...
  it("uses DEFAULT_BENCH_SETTINGS.autoClear when settings are unavailable", () => {
    mockedUseSettings.mockReturnValue({
      settings: undefined,
      isLoading: true,
      updateSettings: vi.fn(),
    ...
    renderTile();
    const expectedHint = DEFAULT_BENCH_SETTINGS.autoClear ? "App default: on" : "App default: off";

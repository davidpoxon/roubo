...
import { getBlueprintVariableGroups } from "./blueprintVariables";

describe("getBlueprintVariableGroups", () => {
  it("returns exactly 4 groups in order: issue, bench, project, config", () => {
    const groups = getBlueprintVariableGroups("global");
  ...
  it("omits config footnote at project scope", () => {
    const groups = getBlueprintVariableGroups("project");
    const configGroup = groups.find((g) => g.category === "config");
    expect(configGroup).toBeDefined();

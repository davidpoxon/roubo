...
import { useBlueprintPreview } from "../../hooks/useBlueprints";
...
function ReadonlyMarkdownViewer({ value }: { value: string }) {
  ...
  return (
    <div
      ...
      data-testid="blueprint-readonly-viewer"
    />
  );
}

export default function BlueprintPreviewPanel({ content, scope, projectId }: Props) {
  ...
  const { data, isPending, isError } = useBlueprintPreview({
    content,

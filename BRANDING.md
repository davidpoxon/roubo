...
**Roubo** (the software) is a local development environment manager — an AI workbench for building software. The name connects the tool to a tradition of precision, craftsmanship, and the empowerment of the maker.
...
All user-facing text must use the Roubo vocabulary. Internal code should also migrate to these terms (see the PRD for phasing).

### Core Terms

| Roubo Term     | Replaces    | Definition                                                                                                                               |
| -------------- | ----------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| **Roubo**      | Workbench   | The product name. Always capitalised as a proper noun. Never "the Roubo" — just "Roubo".                                                 |
| **Bench**      | Slot        | An isolated development environment with its own worktree, ports, and running components. The primary workspace unit. Plural: "benches". |
| **Project**    | Application | A registered repository that defines how benches are configured.                                                                         |
| **Component**  | Service     | A running part of a bench: database, backend, frontend. Components are what you assemble.                                                |
| **Tool**       | Launcher    | A quick-open action: open the browser, launch the IDE, start a shell. Tools are what you reach for.                                      |
| **Inspection** | Testing     | Running quality checks against the work on a bench.                                                                                      |
| **Blueprint**  | Prompt      | A set of instructions for a Claude Code agent working on a bench. A blueprint is a detailed drawing that guides the build.               |

### Configuration File (`roubo.yaml`)

| Section      | Replaces    | Contains                                                            |
| ------------ | ----------- | ------------------------------------------------------------------- |
| `project`    | `app`       | Name, display name, type, repo, GitHub integration.                 |
| `layout`     | `structure` | How the repository is structured: meta-repo, monorepo, single-repo. |
| `components` | `services`  | Component definitions (database, dotnet, vite) with their configs.  |
| `ports`      | `ports`     | Port allocation bases. No rename — ports are universal.             |
| `tools`      | `launchers` | Quick-open actions for browsers and shells.                         |
| `inspection` | `testing`   | Test framework, directory, command, environment.                    |
| `benches`    | `slots`     | Bench limits (e.g., `benches.max: 3`).                              |
| `blueprints` | `prompts`   | Default blueprint and blueprint settings.                           |

### Status Labels

| Roubo Status | Replaces       | When                                                                               |
| ------------ | -------------- | ---------------------------------------------------------------------------------- |
| `idle`       | `inactive`     | Bench exists but nothing is running.                                               |
| `preparing`  | `provisioning` | Bench is being set up: creating workspace, pulling containers, running migrations. |
| `active`     | `running`      | All components are running. Work is underway.                                      |
| `clearing`   | `stopping`     | Components are shutting down. The bench is being tidied.                           |
| `error`      | `error`        | Something went wrong. No rename — clarity trumps metaphor.                         |
...
No deprecated aliases. Old routes are removed entirely — clean break.

| Route                                                         | Replaces                                          |
| ------------------------------------------------------------- | ------------------------------------------------- |
| `/api/projects`                                               | `/api/apps`                                       |
| `/api/projects/:projectId/benches`                            | `/api/apps/:appId/slots`                          |
| `/api/projects/:projectId/benches/:id`                        | `/api/apps/:appId/slots/:id`                      |
| `/api/projects/:projectId/benches/:id/components/:name/start` | `/api/apps/:appId/slots/:id/services/:name/start` |
| `/api/projects/:projectId/benches/:id/components/:name/stop`  | `/api/apps/:appId/slots/:id/services/:name/stop`  |
| `/api/projects/:projectId/benches/:id/components/:name/logs`  | `/api/apps/:appId/slots/:id/services/:name/logs`  |
| `/api/projects/:projectId/benches/:id/blueprints`             | `/api/apps/:appId/slots/:id/prompts`              |
| `/api/projects/:projectId/benches/:id/inspection`             | `/api/apps/:appId/slots/:id/test`                 |
| `/api/benches`                                                | `/api/slots`                                      |
| `/api/containers`                                             | `/api/containers` (unchanged)                     |

### State Storage

No automatic migration from `~/.workbench/`. Users rename the directory manually if they have existing state.

| New Path                 | Replaces                  |
| ------------------------ | ------------------------- |
| `~/.roubo/`              | `~/.workbench/`           |
| `~/.roubo/projects.json` | `~/.workbench/apps.json`  |
| `~/.roubo/state.json`    | `~/.workbench/state.json` |
| `~/.roubo/workspaces/`   | `~/.workbench/worktrees/` |
...
The canonical SVG is at `client/src/assets/roubo-logo.svg`. It uses `currentColor` for flexible theming — the mark inherits whatever colour is set on its parent element.
...
| File                               | Purpose                                                                              |
| ---------------------------------- | ------------------------------------------------------------------------------------ |
| `BRANDING.md`                      | This document. The source of truth for all brand decisions.                          |
| `client/src/assets/roubo-logo.svg` | The canonical logomark SVG. Uses `currentColor`.                                     |
| `CLAUDE.md`                        | Development instructions. Should cross-reference this document for brand compliance. |
| `specs/prd-roubo-rebrand.md`       | The PRD that defines the rebrand scope, requirements, and phasing.                   |
| `roubo.yaml`                       | Per-project configuration file (replaces `workbench.yaml`).                          |
| `schema/roubo-config.schema.json`  | JSON Schema for `roubo.yaml` validation.                                             |

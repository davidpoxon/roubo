...
- **`npm run lint:design-tokens`** fails when a file under `client/src`, or `client/index.html`, uses a raw palette colour utility (`text-stone-500`, `bg-amber-500/15`, `ring-offset-stone-950`, under any colour prefix and any variant chain) or any `dark:` colour variant (`dark:text-stone-300`, `dark:bg-white`, even `dark:bg-bg-surface`). The colour migration (#1293) is complete, so `scripts/design-token-allowlist.json` is empty and the guard covers all of `client/src` and `client/index.html`. Keep it empty: the list only shrinks, an entry that covers no violation fails as stale, and a new entry must never be used to get a violation past the gate. Guard: `scripts/design-token-guard.mjs`.
...
Performance budgets live in `*.perf*.test.*` and `*.scale*.test.*` files beside the code they measure. Their budget assertions are gated behind `RUN_PERF_HARNESS=1`, so the default run skips them (a wall-clock assertion on a shared CI runner is a flake generator) and each file keeps a sentinel plus a non-gated structural test so it still asserts something. `npm run test:perf` sets the flag, runs those files alone and one at a time (`--no-file-parallelism`, so a wall-clock sample is never taken while another file is starting up on the same cores), and prints each one's `perf-evidence` JSON. A render harness warms up with several unmeasured renders before it samples: one is not enough to reach steady state, and p95 over 20 samples then reports a cold render. Keep testing-library role queries out of the timed region, since they walk the whole accessibility tree and can cost as much as the render being measured. Both filename patterns are in the script's filter, so a new budget harness only gets run if its filename carries one of them. The `perf` workflow runs `npm run test:perf` on every merge to `main` (and on demand via `workflow_dispatch`). Because it runs post-merge it gates nothing: no PR is ever blocked by a wall-clock budget, and a breach instead turns `main` red. Each run's `perf-evidence` output is uploaded as a retained artifact, including when the run fails, so the numbers behind a breach are there to read.

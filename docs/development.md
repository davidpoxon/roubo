...
[DESIGN.md](../DESIGN.md) records colour as roles, not shades, and the client is built to it. Three gates in the `pr-check` `lint` job keep the tree in line with it: two over the client, one over the generated token file itself. All three read committed files only, so they need no install.
...
- **`npm run lint:emitted-tokens`** fails when the DESIGN.md token block and the generated `design-tokens/tokens.tailwind.css` disagree: a token DESIGN.md records that the `@theme` block never emitted, a `--*` the block still emits that DESIGN.md dropped, or a value that changed (an `alpha` included, which the emitter bakes into an 8 digit hex, so `0.4` becomes `#F59E0B66`). That file is generated by the ui-design `emit_tokens.py` and committed by hand, so a half-finished emit otherwise merges green: the build compiles and the utility naming the variable that never arrived paints nothing. Fix it by re-running the emitter and committing the result, not by hand editing the file. The failure prints the exact lines to add, remove or replace. Guard: `scripts/emitted-tokens-guard.mjs`.

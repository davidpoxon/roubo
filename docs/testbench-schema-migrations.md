...
- The lifecycle block is excluded from the canonical case body, so retiring or superseding a case changes neither the plan hash nor that case's changed/unchanged classification. The plan hash is therefore a hash of the **testable** plan, not of the file: it covers titles, levels, preconditions, steps, and observations, and deliberately ignores lifecycle bookkeeping. A lifecycle-only edit raises no stale-results warning and moves no verify gate to `stale`, while a genuine content edit still does. The consequence to plan for: once the gate surface honours the lifecycle block, a retired case will stop being unresolved, so a gate will be able to move from pending to passed with no staleness prompt at all. The hash cannot flag that, which is why the gate surface, not the hash, is where lifecycle exclusions have to be shown. As of #1164 `evaluateGate` does honour `lifecycle`: a retired case leaves the effective gating set, a superseded case's status is read from the case its replacement chain resolves to, an unresolvable pointer keeps the case in the set as unresolved, and a gating set emptied by lifecycle reports a reason distinct from one emptied by the level and type policy.

## test-results.json

### 1.0.0 to 2.0.0 (benches-map flatten)

Before 2.0.0, a single `test-results.json` held results for many benches nested under a per-bench `benches` map. As of 2.0.0 (the #494 flatten), one results file lives per worktree (a sibling of `test-cases.json`), so a file holds exactly one bench's results and `caseResults` sits at the TOP LEVEL instead of nested under `benches`.

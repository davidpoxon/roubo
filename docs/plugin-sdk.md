...
`minVersion` and `testedCeiling` are optional and each must be a bare `major.minor.patch` version: not a range, not `v`-prefixed, and no prerelease or build-metadata suffix. The whole block is optional, so integration and component manifests, and agent manifests that omit it, validate unchanged. One rule covers both places the window is declared: the descriptor enforces exactly the same shape (see [The version probe and its gate](#the-version-probe-and-its-gate)), because the host cannot compare anything else.
...
Roubo's server does not always inherit the `PATH` a user's shell has. A per-user shim and a Finder or Dock launch both produce a server whose `PATH` is missing the directory your CLI lives in, and a descriptor declaring a bare `command` then fails to launch on exactly those installs. `agentInstallLocations` is how you fix that for your own CLI:
...
The control stays in the keyboard tab order in every state, and the status line is announced to assistive technology. A value saved earlier is kept while the field is loading or failed; the form only stops offering a way to change it. On a project's agent overrides, a probed field with no app default and no `default` of its own cannot be switched to an override until its choices arrive, because the override would have no value to start from.
...
The `config` your `translateLaunch` receives is whatever the user saved for your plugin, and your manifest `configSchema` is the only thing that decides what that form looks like. Roubo renders the schema's top-level properties on the **Settings > AI Agents** screen: a `string` becomes a text field, a `boolean` a checkbox, a `number` or `integer` a number field, and a property with an `enum` (or a `oneOf` of `const` branches, each of which may carry its own `title`) becomes a select of exactly those values. Give each property a `title` and a `description` and they become the control's label and help text. The host validates every save against the same schema and refuses an out-of-range value, so `translateLaunch` can trust that its `config` conforms. A property can carry its own `errorMessage` string alongside its other JSON Schema keywords (for example, next to a `pattern`); the refusal returned to the form uses that text verbatim instead of Ajv's default message for the keyword that failed. This is most useful on a `pattern`, where Ajv's own message is the raw regex: say what the field actually wants, in the field's own terms, rather than showing a reviewer a pattern to reverse-engineer. Leave it off and the refusal falls back to Ajv's default, exactly as before; a host that predates this reads and ignores the unknown keyword the same way, so no `roubo: ^x.y.z` host-API version floor is needed to use it. Each agent plugin's saved defaults live in their own file, `~/.roubo/agents/_global/<pluginId>.yaml`, so nothing you declare can collide with another agent plugin's configuration.
...
```json
{
  ...
  "type": "module",
  "scripts": { "build": "tsc" },
  "dependencies": { "@roubo/plugin-sdk": "0.8.0" },
...
| `probe-failed`         | Binary unresolvable, `command` still templated, or output has no version | Launches, with a notice saying the check did not run. A probe that cannot decide never blocks.                     |

Both bounds are inclusive and both are optional, and each must be a bare `major.minor.patch` version: not a range, not `v`-prefixed, and no prerelease or build-metadata suffix. The host compares a bound to the detected version numerically, one dot-separated segment at a time, so anything outside that shape is uncomparable. Such a bound is rejected when the manifest or the descriptor is validated, naming the offending field, rather than silently reading as a floor nothing satisfies. A probe with no `minVersion` can never block, and one with no `testedCeiling` can never warn. The ceiling is deliberately non-blocking, because agent CLIs ship weekly and refusing to run on an unrecognised newer version would age far worse than a warning does. It still earns its keep: when a launch above the ceiling fails, the host attributes the failure to a probably-stale argument map rather than leaving the user guessing.
...
| `workspace-unavailable` | The bench's workspace directory does not exist on disk yet (checked before any workspace write or the spawn) | "Could not start: the bench's workspace does not exist yet", telling the user to wait for the bench to finish preparing and retry. This is the same exit-nonzero-no-output shape `missing-binary` detects after the spawn, caught earlier here because the cause is the directory, not your CLI. |
...
- No em dashes (U+2014) in any user-facing copy. The project lint enforces it. En dashes (`–`) are fine for numeric ranges only.
...
- **When the tier is active, the container gets only the plugin's own declared environment, never the host's.** A plugin process never reads `process.env`: credentials arrive through `host.credentials.get(slot)` and outbound requests through `host.fetch`, so the only environment the container needs is the two host-set identity variables, `ROUBO_PLUGIN_ID` and `ROUBO_HOST_API_VERSION`. The host's own environment, including whatever credentials the launching shell holds, stays scoped to the `docker` CLI process that needs it to reach the daemon, and is never forwarded into the container.

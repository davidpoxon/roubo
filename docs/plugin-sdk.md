...
| `permissions.processes`         | `false` or object                       | `false` for no child processes, or `{ executables: string[] }` listing the executables the plugin may spawn. Enforced: `host.process.spawn` rejects any executable not on the list. Agent plugins must declare `false`; a `processes` block on a `kind: agent` manifest is rejected at validation, because a spawned child would sidestep the declarative launch descriptor |

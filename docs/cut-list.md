...
- **Filter facets reflect the primary source only.** The label, issue-type, and other facet options offered in the cut list's filter UI are currently derived from the first source, not the union of all sources. In a multi-source project a label that exists only in a submodule may not be offered as a filter even though items carrying it appear in the list.

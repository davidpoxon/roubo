...
// The imperative (escape-hatch) one-shot deploy lifecycle, factored out of the
// registration entrypoint so it can be unit-tested against a mocked host. The
// journey: the start hook gates on the broker capability, runs a single command
// to completion through the host, and reports `completed`.
...
export function buildContract(host: ComponentHostClient): ImperativeComponentContract {
  ...
  return {
    async start(context: BenchContext): Promise<void> {
      ...
      // Graceful version gate (CP-FR-017): ask the host whether the broker
      // method this plugin depends on is available before driving it. On a host
      // too old to expose `host.process.run`, `available` is false and the
      // plugin degrades rather than crashes; on a compatible host it is true.

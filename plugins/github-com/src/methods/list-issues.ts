...
function parseProjectExternalId(externalId: string): { owner: string; projectNumber: number } {
  ...
  if (hashIdx === -1) {
    throw new Error(
      `[github-com] project externalId "${externalId}" missing "#<number>". Expected "owner/#1".`,
    );
  }
  ...
  if (!owner || !Number.isInteger(projectNumber) || projectNumber <= 0) {
    throw new Error(
      `[github-com] project externalId "${externalId}" not in the expected "owner/#<positive-int>" form.`,
    );
  }
  return { owner, projectNumber };
}

function alertFlagsOf(source: GithubSource): AlertFlags {
  return {
